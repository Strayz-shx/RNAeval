...
#%%
# Training data
train_x_color_mat_np, train_x_nt_localized_info_mat_np, train_y_np, train_x_embedding, train_x_ss = data_prep_RNA_mat_3d_nt_localized_info_mat_PDB_data(Original_RNA_Data = Original_RNA_Data_train, padding_length=410.0)
...
# Validation data
val_x_color_mat_np, val_x_nt_localized_info_mat_np, val_y_np, val_x_embedding, val_x_ss = data_prep_RNA_mat_3d_nt_localized_info_mat_PDB_data(Original_RNA_Data = Original_RNA_Data_val, padding_length=410.0)
...
# NN_model_utilized = torch.nn.DataParallel(NN_model_utilized, device_ids=[0, 1])  # 两个模型进行训练
...
training_data_utilized = dataloader_prep_with_sampler(x_color_mat_stack_4D_tensor = train_x_color_mat_4D_torch,
                                                      ...
                                                      x_ss=train_x_ss)
# training_data_utilized = dataloader_prep(x_color_mat_stack_4D_tensor = train_x_color_mat_4D_torch,
#                                          x_nt_localized_info_mat_stack_4D_tensor = train_x_nt_localized_info_mat_4D_torch,
#                                          y_tensor = train_y_torch,
#                                          batch_size_value = batch_size_value_utilized,
#                                          x_embedding=train_x_embedding,
...
figure_name_utilized = 'NU_ResNet_batch_'+str(batch_size_value_utilized)+'_lr_'+str(learning_rate_value_utilized)+\
    '_weightd_'+str(weight_decay_value_utilized)+'_epochs_'+str(num_epochs_utilized)+'_scheduler_gamma_'+\
        str(gamma_value_utilized)+'_PDB_data'


train_val_figure_plot_function(output_dir=r"/home/Shihx/rnadataloder/NU-renet/output_inception",
                               figure_name = figure_name_utilized,
