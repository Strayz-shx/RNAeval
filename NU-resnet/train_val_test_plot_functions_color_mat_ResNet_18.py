...
def dataloader_prep_with_sampler(x_color_mat_stack_4D_tensor, x_nt_localized_info_mat_stack_4D_tensor, y_tensor, x_embedding, x_ss, batch_size_value, sampler_applied):
    
    data_applied = torch.utils.data.TensorDataset(x_color_mat_stack_4D_tensor, x_nt_localized_info_mat_stack_4D_tensor, y_tensor, x_embedding, x_ss)
...
def dataloader_prep(x_color_mat_stack_4D_tensor, x_nt_localized_info_mat_stack_4D_tensor, y_tensor, x_embedding, x_ss, batch_size_value):
...
def train_val_ResNet_expert_color_mat(device, NN_model, num_epochs, optimizer, scheduler_for_optimizer, loss_function, model_name, training_data_used, val_data_used):
    ...
    for epoch in range(num_epochs):
        ...
        for i, mat_label in enumerate(training_data_used):
            ...
            color_mat_train, nt_localized_info_mat_train, label_train, embedding, ss = mat_label[0].to(device), mat_label[1].to(device), mat_label[2].to(device), mat_label[3].to(device), mat_label[4].to(device)
            optimizer.zero_grad() 
            # pred_train = NN_model(color_mat_train)
            pred_train = NN_model(embedding, ss)
        ...
        with torch.no_grad():
            for i, mat_label in enumerate(val_data_used):
                ...
                color_mat_val, nt_localized_info_mat_val, label_val, embedding, ss = mat_label[0].to(device), mat_label[1].to(device), mat_label[2].to(device), mat_label[3].to(device), mat_label[4].to(device)
                # pred_val = NN_model(color_mat_val)
                pred_val = NN_model(embedding, ss)
...
def test_ResNet_expert(device, NN_model, loss_function, model_name, testinging_data_used, testing_data_name):
    ...
    with torch.no_grad():
        for i, mat_label in enumerate(testinging_data_used):
            color_mat_test, nt_localized_info_mat_test, label_test, embedding, ss = mat_label[0].to(device), mat_label[1].to(device), mat_label[2].to(device), mat_label[3].to(device), mat_label[4].to(device)
            # pred_test = NN_model(color_mat_test)
...
def train_val_figure_plot_function(output_dir, figure_name, loss_value_train, loss_value_val, accuracy_train, accuracy_val, auc_roc_train, auc_roc_val, auc_roc_train_check, auc_roc_val_check):
    # 确保输出文件夹存在
    ...
    # 构建保存路径
    figure_name_loss_value_train_val = os.path.join(output_dir, figure_name + '_loss_value_train_validation_data.jpg')
    figure_name_accuracy_train_val = os.path.join(output_dir, figure_name + '_accuracy_train_validation_data.jpg')
    figure_name_accuracy_val = os.path.join(output_dir, figure_name + '_accuracy_validation_data.jpg')
    figure_name_auc_roc_train_val = os.path.join(output_dir, figure_name + '_auc_roc_train_validation_data.jpg')
    figure_name_auc_roc_train_val_check = os.path.join(output_dir,
                                                       figure_name + '_auc_roc_train_validation_data_check.jpg')
